...
int main( int argc, char** argv )
{
    ...
    try
    {
        ...
        if( useVVF )
        {
            ...
            try
            {
                volume = std::make_shared< nogo::MaskVolume >( settings.volumeSize, settings.volumeSize, settings.volumeSize );
        ...
        if( useEVD )
        {
            try
            {
                ...
                auto vsize = ( settings.evdVolumeSize > 0 ) ? std::min( settings.volumeSize, settings.evdVolumeSize ) : settings.volumeSize;
            ...
            catch( std::exception& e )
            {
                ...
                if( useVVF )
                {
                    ...
                    dtVolume = nogo::dt( *volume, settings.evdVolumeSize, settings.evdScaleSplineOrder, settings.evdScaleThreshold);
                    dtCalced = true;
                }
                else
                {
                    LogE << "Cannot calculate EVD without a mask volume. Specify mask volume file to force EVD calculation." << LogEnd;
                    return -1;
                }
            }
