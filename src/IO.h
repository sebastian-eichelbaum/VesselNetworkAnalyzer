...
namespace nogo
{
    ...
    void writePyArrays( const std::string filename, const nogo::VesselNetwork& vessels )
    {
        ...
        out << "# Points. Index referenced by lines. Each point is an array of [x,y,z,radius,uses]. Uses is the amount of connected edges." << std::endl;
    ...
    void writeLineStrips( const std::string filename, const PointContainerType& points, const LineStripContainer& lineStrips,
                          const SegmentData& segmentData, const BranchPointData& branchPointData,
                          std::function< bool( const VesselNetwork::IndexType& ) > accept = []( const VesselNetwork::IndexType& /* index */ )
                          {
                              return true;
                          } )
    {
        ...
        out << "# -> L Index_0 Index_1 Index_... -- this is a line-strip consisting of the denoted point indices." << std::endl;
        out << "# " << std::endl;
        out << "# * Besides these mandatory tokens, there can be:" << std::endl;
        out << "# -> PA Num Value -- Point attribute. Num defines the number of the attribute (not the point index), starts at 0." << std::endl;
        out << "# -> LA Num Value -- Line attribute. Num defines the number of the attribute (not the line index), starts at 0." << std::endl;
        out << "# -> Name Token String -- Name of a token. Tokens as above, L, P, PA Num, LA Num, ..." << std::endl;
        out << "# -> Description Token String -- Description of a token. Tokens as above, L, P, PA Num, LA Num, ..." << std::endl;
        out << "# Now, here is the data you are waiting for ..." << std::endl;
        out << "# " << std::endl;
        out << "# " << std::endl;
        out << "# Names: " << std::endl;
        out << "Name PA 0 " << "isBranchPoint" << std::endl;
        out << "Description PA 0 " << "An attribute describing whether the point is a BranchPoint ( = 1 ) or not." << std::endl;
        out << "Name PA 1" << " isCapillary" << std::endl;
        out << "Description PA 1 " << "An attribute describing whether the point is a capillary ( = 1 ) or not." << std::endl;
        out << "Name PA 2" << " Degree" << std::endl;
        out << "Description PA 2 " << "The branchpoint degree. Non-branchpoints have this set to 0." << std::endl;

        out << "Name LA 0 " << "Radius" << std::endl;
        out << "Description LA 0 " << "The radius of the segment." << std::endl;
        out << "Name LA 1 " << "isCapillary" << std::endl;
        out << "Description LA 1 " << "An attribute describing whether the line is a capillary ( = 1 ) or not." << std::endl;
        out << "# " << std::endl;
        out << "# " << std::endl;
        out << "# Data: " << std::endl;
        ...
        for( auto stripIdx : range( lineStrips.size() ) )
        {
            ...
            {
                out << static_cast< size_t >( pointIdx ) << " ";    // NOTE: the static cast ensures that we do not print chars if the
                                                                    // index type might some uint8 or something (which is unlikely
                                                                    // but might happen though).
            }
            out << std::endl;
     ...
     *
     * \tparam ContainerType the type of container to print. Basically this can be anything that is iteratable and whose contents provide a
     * operator.
     * \tparam KeyContainerType the type of container to print (as keys). Basically this can be anything that is iteratable and whose contents
     * provide a << operator.
     * \param filename the file where to write to.
     * \param container the container to write
     * \param keyContainer the key-container to write. Needs to have the same size as container, or is empty (no keys written)
     * \param prefix a prefix per value. Empty by default.
    ...
    void writeKeyValueList( const std::string filename, const KeyContainerType& keyContainer, const ContainerType& container,
                         std::function< bool( const VesselNetwork::IndexType& ) > accept = []( const VesselNetwork::IndexType& /* index */ )
                         {
                             return true;
                         },
    ...
    void writeValueList( const std::string filename, const ContainerType& container,
                         std::function< bool( const VesselNetwork::IndexType& ) > accept = []( const VesselNetwork::IndexType& /* index */ )
                         {
                             return true;
                         },
    ...
    void write( const std::string baseDir, const SegmentData& data )
    {
        ...
        writeValueList( filenameBase + "lengths.capillary.csv", data.lengths, [ & ]( auto index ){ return data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "lengths.non-capillary.csv", data.lengths, [ & ]( auto index ){ return !data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "lengths.csv", data.lengths );

        // for all three cases: capillary, non-capillary, both
        writeValueList( filenameBase + "tortuosity.capillary.csv", data.tortuosity, [ & ]( auto index ){ return data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "tortuosity.non-capillary.csv", data.tortuosity, [ & ]( auto index ){ return !data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "tortuosity.csv", data.tortuosity );

        // for all three cases: capillary, non-capillary, both
        writeValueList( filenameBase + "radius.capillary.csv", data.radius, [ & ]( auto index ){ return data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "radius.non-capillary.csv", data.radius, [ & ]( auto index ){ return !data.isCapillary[ index ]; } );
        ...
        // for all three cases: capillary, non-capillary, both
        writeValueList( filenameBase + "diameter.capillary.csv", data.diameter, [ & ]( auto index ){ return data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "diameter.non-capillary.csv", data.diameter, [ & ]( auto index ){ return !data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "diameter.csv", data.diameter );

        // for all three cases: capillary, non-capillary, both
        writeValueList( filenameBase + "volume.capillary.csv", data.volume, [ & ]( auto index ){ return data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "volume.non-capillary.csv", data.volume, [ & ]( auto index ){ return !data.isCapillary[ index ]; } );
        ...
        writeValueList( filenameBase + "density.non-capillary.csv", std::vector< Real >( 1, data.segmentDensityNonCapillary ) );
        ...
        // Bins
        writeKeyValueList( filenameBase + "diameter_per_length_hist.csv", data.meanDiameterPerLengthBinCenters, data.meanDiameterPerLength );
    }
    ...
    void write( const std::string baseDir, const BranchPointData& data )
    {
        ...
        writeValueList( filenameBase + "degrees.capillary.csv", data.degrees, [ & ]( auto index ){ return data.isCapillary[ index ]; } );
        writeValueList( filenameBase + "degrees.non-capillary.csv", data.degrees, [ & ]( auto index ){ return !data.isCapillary[ index ]; } );
        ...
        writeValueList( filenameBase + "degrees_mean.non-capillary.csv", std::vector< Real >( 1, data.degreeMeanNonCapillary ) );
        ...
        // Densities
        writeValueList( filenameBase + "density.capillary.csv", std::vector< Real >( 1, data.branchPointDensityCapillary ) );
        writeValueList( filenameBase + "density.non-capillary.csv", std::vector< Real >( 1, data.branchPointDensityNonCapillary ) );
        writeValueList( filenameBase + "density.csv", std::vector< Real >( 1, data.branchPointDensity ) );

        writeValueList( filenameBase + "density.degree3.capillary.csv", std::vector< Real >( 1, data.branchPointDegree3DensityCapillary ) );
        writeValueList( filenameBase + "density.degree3.non-capillary.csv", std::vector< Real >( 1, data.branchPointDegree3DensityNonCapillary ) );
        writeValueList( filenameBase + "density.degree3.csv", std::vector< Real >( 1, data.branchPointDegree3Density ) );

        writeValueList( filenameBase + "density.degree4.capillary.csv", std::vector< Real >( 1, data.branchPointDegree4DensityCapillary ) );
        writeValueList( filenameBase + "density.degree4.non-capillary.csv", std::vector< Real >( 1, data.branchPointDegree4DensityNonCapillary ) );
    ...
    void write( const std::string baseDir, const VolumeData& data )
    {
        ...
        // Densities
        writeValueList( filenameBase + "vvf_local.capillary.csv", std::vector< Real >( 1, data.vesselVolumeFractionLocalCapillary ) );
        writeValueList( filenameBase + "vvf_local.non-capillary.csv", std::vector< Real >( 1, data.vesselVolumeFractionLocalNonCapillary ) );
                ...
                const SegmentData& segmentData, const BranchPointData& branchPointData, std::string filenamePrefix = "" )
    {
        ...
        LogI << "Writing vessel segments to \"" << filename << "\"." << LogEnd;
        writeLineStrips( filename, points, segments.segments, segmentData, branchPointData, [&]( auto&& segmentIdx )
                         {
                             return !segments.removed[ segmentIdx ];
                         } );
    }
    ...
    inline SPtr< VesselNetwork > loadVesselNetworkVTK( const std::string& filename, double scale = 1.0 )
    {
        ...
        // NOTE: the first 5 lines of a VTK file are ASCII, regardless of binary or ascii data modes. Read and check validity:
        std::string line;
        ...
        while( input.good() && !input.eof() )
        {
            ...
            if( elems[ 0 ] == "POINTS" )
            {
                if( elems.size() != 3 )
                {
                    throw std::runtime_error( "File \"" + filename + "\" contains invalid Points block header \"" + line + "\"." );
                }
                ...
                // According to the VTK doc, a block ends with a line break. Skip the break and continue reading the next block.
                input.ignore();
            }
            ...
            else if( elems[ 0 ] == "CELL_DATA" ) // Is additional data per cell??
            {
                if( elems.size() != 2 )
                {
                    throw std::runtime_error( "File \"" + filename + "\" contains invalid Cell Data block header \"" + line + "\"." );
                }
                ...
                {
                    throw std::runtime_error( "File \"" + filename + "\" has a cell data block with unsupported field type: \"" + line + "\"" );
                }
                ...
                LogD << "Field Data found: " << fieldElems[ 0 ] << " with " << fieldElems[ 2 ] << " values of type " << fieldElems[ 3 ] << LogEnd;
            ...
            else if( elems[ 0 ] == "POINT_DATA" ) // Ignore it parse point data?
            {
                if( elems.size() != 2 )
                {
                    throw std::runtime_error( "File \"" + filename + "\" contains invalid Point Data block header \"" + line + "\"." );
                }
                ...
                if( line != "FIELD FieldData 1" || !input.good() )
                {
                    throw std::runtime_error( "File \"" + filename + "\" has a point data block with unsupported field type: \"" + line + "\"" );
                }
        ...
        if( !( hasRadii && hasPoints && hasLines ) )
        {
            throw std::runtime_error( "File \"" + filename + "\" does not contain one of the blocks required. POINTS: " +
                                      std::to_string( hasPoints ) + " LINES: " + std::to_string( hasLines ) + " RADII: " +
                                      std::to_string( hasRadii ) + "." );
        }
        ...
        Vec3 bbMin = { { std::numeric_limits< VesselNetwork::ValueType >::max(), std::numeric_limits< VesselNetwork::ValueType >::max(),
                         std::numeric_limits< VesselNetwork::ValueType >::max() } };
        Vec3 bbMax = { { std::numeric_limits< VesselNetwork::ValueType >::min(), std::numeric_limits< VesselNetwork::ValueType >::min(),
        ...
        // We now have the line-strips as a block of run-length encoded indices. For a more comfortable use, we re-arrange it a bit. -> inflate the
        // run-length encoding.
        size_t idx = 0;
        // size_t lIdx = 0;
        ...
        while( loadedLines.size() > idx )
        {
            ...
            for( const auto pI : range( len - 1 ) )
            {
                ...
                {
                    throw std::runtime_error( "File \"" + filename + "\" contains lines with point indices out of range: " + std::to_string( vI1 ) +
                                              " >= " + std::to_string( result->points.size() ) + " for " + std::to_string( pI ) + "." );
                }
                if( static_cast< size_t >( vI2 ) >= result->points.size() )
                {
                    throw std::runtime_error( "File \"" + filename + "\" contains lines with point indices out of range: " + std::to_string( vI2 ) +
                                              " >= " + std::to_string( result->points.size() ) + " for " + std::to_string( pI ) + "." );
                }

                result->pointRadii[ vI1 ] = scale * static_cast< VesselNetwork::ValueType >( byteSwap( loadedPointRadii[ vI1 ] ) );
                result->pointRadii[ vI2 ] = scale * static_cast< VesselNetwork::ValueType >( byteSwap( loadedPointRadii[ vI2 ] ) );
        ...
        while( loadedLines.size() > idx )
        {
            ...
            if( len != 2 )
            {
                throw std::runtime_error( "File \"" + filename + "\" contains line-strips with more than one segment. This is not supported." );
            }
            ...
            {
                throw std::runtime_error( "File \"" + filename + "\" contains lines with point indices out of range: " + std::to_string( i1 ) +
                                          " >= " + std::to_string( result->points.size() ) + "." );
            }
            if( static_cast< size_t >( i2 ) >= result->points.size() )
            {
                throw std::runtime_error( "File \"" + filename + "\" contains lines with point indices out of range: " + std::to_string( i2 ) +
                                          " >= " + std::to_string( result->points.size() ) + "." );
            }
    ...
    {
        // Open the file
        std::ifstream f( filename, std::ios::in | std::ios::ate | std::ios::binary ); // NOTE: ios::ate sets pointer to end of file
        if( !f.good() )
        {
