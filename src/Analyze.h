...
namespace nogo
{
    ...
    /**
     * Cut the given bounding box using a given percentage. The cut operation is done along the centerline of the cylinder represented by the bb.
     *
    ...
    {
        // Now cut the cylinder. Keep in mind that the data provided always uses a cylinder with xy planes and z as height.
        auto a = bb.first;
        auto b = bb.second;
        // How much?
        ...
        return std::make_pair(
            Vec3( { { a[ 0 ] + ( 0.5 * frac * dX ), a[ 1 ] + ( 0.5 * frac * dY ), a[ 2 ] + ( 0.5 * fracZ * dZ ) } } ),
    ...
    auto volume( const BoundingBox& bb, bool cylindric )
    {
        ...
        return ( bb.second[ 0 ] - bb.first[ 0 ] ) * ( bb.second[ 1 ] - bb.first[ 1 ] ) *
               ( bb.second[ 2 ] - bb.first[ 2 ] );
    }
    ...
    /**
     * Calc an intersection with the cylinder between p1 and p2. P2 needs to be outside. The intersection is done via approximation, not exact.
     *
    ...
    Vec3 cylinderIntersection( const Vec3& p1, const Vec3& p2, const BoundingBox& bb, size_t count = 0 )
    {
        ...
        // Since the lines where beautified using splines, they are  sampled densely. The center point is probably almost near the border.
     ...
     */
    VesselSegments deriveNetwork( const decltype( VesselNetwork::points )& points, const decltype( VesselNetwork::lines )& lines,
                                  const decltype( VesselNetwork::radii )& radii, bool splitAtDiffRadii, bool mergeCloseBranchPoints,
                                  Real maxMergeDistance )
    {
        ...
        // Gather information about each point's degree - how many lines are adjacent to this point? Index matches point index.
        ...
        for( auto lineIdx : range( lines.size() ) )
        {
            ...
            // NOTE: keep in mind that this will be equal to linesByPoint[ p1( or 2)Idx ].size(). But as we might need this value quite often, we
            // store it separately to avoid multiple size() calls.
        ...
        {
            // A point is NOT an endpoint if it is adjacent to 2 lines and both lines have the same radius. -> accordingly, it is an endpoint if this
            // is not true.
        ...
        // Lets use an in-line function for this. If we would write our own function, it would need a whole bunch of additional parameters for the
        // lines vector, points vector linesByPoint .... and so on.
        //
        // This method also associates the originally given radius to each segment. It compares the radius to the expected radius, as the data SHOULD
        // provide a single radius for all lines of a single segment. Fails if this is not the case!
        //
        // The function returns true if there has been an issue with multiple radii at a single segment.
        std::function< Real( bool, VesselSegments::Segment&, const VesselNetwork::IndexType&, const VesselNetwork::IndexType&,
                             VesselSegments::SegmentRadii& )> traceSegment =
                 ...
                 const VesselNetwork::IndexType& lineIdx, VesselSegments::SegmentRadii& segRadii )
        {
            ...
            // Some datasets contain cycles. To prevent endless looping, check if the next point was visited already.
            ...
            // We check AFTER adding the point. This way, the first and last point of the segment are equal (closed loop). This is more accurate for
        ...
        // Keep in mind that this routine might change the order of points in a line. So a line defined by (a,b), might be represented here as (b,a)
        // as we cannot make any assumptions on the ordering of points in the source data.
        ...
        // Clean up some special cases of parallel segments with only two points. There are a lot of cases, where several lines connect the same
        // points. Remove them.
        size_t remCount = 0;
        ...
        LogI << "Found " << vessels.segments.size() << " vessel segments. Removed " << remCount << " as they where equal." << LogEnd;
        ...
        if( mergeCloseBranchPoints )
        {
                ...
                for( auto segmentIdx : range( vessels.segments.size() ) )
                {
                    ...
                    if( ( l < maxMergeDistance ) && ( vessels.degrees[ frontIdx ] > 2 ) && ( vessels.degrees[ backIdx ] > 2 ) )
                    {
                        ...
                        for( auto segmentIdxInvolved : range( vessels.segments.size() ) )
                        {
                            ...
                            // There is a special case to handle: Some segments might now get obsolete, as they now use the newIdx point as start and
                            // end. Remove them.
                            ...
                            else if( needLengthUpdate )
                            {
                                ...
                                {
                                    LogE << vessels.segments[ segmentIdxInvolved ].size() << " - " << frontIdxInvolved << " --- " << backIdxInvolved
                                         << " has negative length! This should not happen. Current = " << currentLength << " minus old " << oldD
                                         << " plus new " << newD << LogEnd;
                                }
                            }
                        }
                    }
            ...
            LogD << "Merged " << mergeCount << " segments in " << numMergeIterations - 1 << " iterations. Removed " << numRemoved
                 << " due to equality to merged vessels." << LogEnd;
        }
        ...
        for( auto item : vessels.adjacency )
        {
            ...
            if( diff != 0 )
            {
                //LogW << pIdx << " has different degrees in adjacency - calculated: " << adj.size() << " - " << vessels.degrees[ pIdx ] << " = "
                //     << diff << LogEnd;
                                                        ...
                                                        bool cylindric, ValueType outsideValue = ValueType() )
    {
        ...
        // Just create a plain copy. Be aware, that this does not include the cylinder test. This needs to be done during calculations.
                  ...
                  ConstSPtr< DTVolume > dtVolume, const Settings& settings )
    {
        ...
        // IF you change this to be something different from the network cut, also adapt the below calculations that use the network volume
        ...
        LogD << "Cut Network Bounding Box: [ ( " << cutNetworkBB.first[ 0 ] << ", " << cutNetworkBB.first[ 1 ] << ", " << cutNetworkBB.first[ 2 ]
             << " ), ( " << cutNetworkBB.second[ 0 ] << ", " << cutNetworkBB.second[ 1 ] << ", " << cutNetworkBB.second[ 2 ] << " ) ]" << LogEnd;
        ...
        // Important: this code avoids actually preprocessing the line data. Instead, the below code uses the ignore-predicate to check at each case
        // whether to include the point into the statistics or not.
        ...
        LogD << "Original Network Info: " << origPoints.size() << " points in " << origLines.size() << " lines." << LogEnd;
        ...
        auto vessels = deriveNetwork( points, lines, radii, splitSegmentsAtDiffRadii, mergeCloseBranchPoints, maxMergeDistance );
        ...
        Real diameterPerLengthBinWidth = ( diameterPerLengthMax - diameterPerLengthMin ) / static_cast< Real >( diameterPerLengthNumBins );
        ...
        for( auto segmentIdx : range( vessels.segments.size() ) )
        {
            ...
            // All containers have the same size. As we push back, but skip some segments, segmentIDX and the real index in these containers may be
            // different -> store in a map
        ...
        localSegmentData.segmentDensityNonCapillary = static_cast< Real >( numNonCapillarySegments ) / networkCutDomainVolume;
        ...
        {
            size_t binNum = 0;
            std::generate( localSegmentData.meanDiameterPerLength.begin(), localSegmentData.meanDiameterPerLength.end(),
                [&]()
                {
                    // Calc mean:
                    auto mean = std::accumulate( diameterPerLenght[ binNum ].begin(), diameterPerLenght[ binNum ].end(), Real() );
        ...
        {
            size_t binNum = 0;
            std::generate( localSegmentData.meanDiameterPerLengthBinCenters.begin(), localSegmentData.meanDiameterPerLengthBinCenters.end(), [&]()
                {
        ...
        LogD << numSegmentsNotBetweenBranchpoints << " of " << numSegments << " segments are not between branchpoints." << LogEnd;
        ...
        for ( auto item : adjacentBifVectors )
        {
            ...
            std::sort( adjBifVectors.begin(), adjBifVectors.end(), []( const auto& a, const auto& b )
                    {
            ...
            for( auto adjVecIdx : range( adjBifVectors.size() ) )
            {
                ...
                for( auto relVecIdx : range( adjVecIdx + 1, adjBifVectors.size() ) )
                {
                    ...
                    localSegmentData.anglesAdjacency.addByDiameter( relVec.radius * 2.0, angle, isCapillary( relVec.radius ) );
                }
            }
        }
        ...
        for( auto segmentIdx : range( vessels.segments.size() ) )
        {
            ...
            for( auto pIdx : std::vector< VesselNetwork::IndexType >{{ p1Idx, p2Idx }} )
            {
                if( !handledPoint[ pIdx ] )
                {
                    ...
                    {
                        // Keep a map between the real point index and the branch point index used in the branchPointData struct
                        ...
                        localBranchPointData.branchPointDegree3DensityCapillary += ( isACapillary && ( degree == 3 ) ) ? 1.0 : 0.0;
                        localBranchPointData.branchPointDegree4DensityCapillary += ( isACapillary && ( degree == 4 ) ) ? 1.0 : 0.0;

                        localBranchPointData.branchPointDensityNonCapillary += ( !isACapillary ) ? 1.0 : 0.0;
                        localBranchPointData.branchPointDegree3DensityNonCapillary += ( !isACapillary && ( degree == 3 ) ) ? 1.0 : 0.0;
                        localBranchPointData.branchPointDegree4DensityNonCapillary += ( !isACapillary && ( degree == 4 ) ) ? 1.0 : 0.0;
        ...
        localBranchPointData.branchPointDegree3Density =
            ( localBranchPointData.branchPointDegree3Density > 0.0 ) ? localBranchPointData.branchPointDegree3Density / networkCutDomainVolume : 0.0;
        localBranchPointData.branchPointDegree4Density =
            ( localBranchPointData.branchPointDegree4Density > 0.0 ) ? localBranchPointData.branchPointDegree4Density / networkCutDomainVolume : 0.0;
        ...
        LogContinue << LogEnd;
        LogI << "Branch Degree Means - all: " << localBranchPointData.degreeMean << " - capillary: " << localBranchPointData.degreeMeanCapillary
        ...
        else
        {
            ...
            LogD << "Cut Volume Bounding Box: [ ( " << cutVolumeVVFBB.first[ 0 ] << ", " << cutVolumeVVFBB.first[ 1 ] << ", "
                 << cutVolumeVVFBB.first[ 2 ] << " ), ( " << cutVolumeVVFBB.second[ 0 ] << ", " << cutVolumeVVFBB.second[ 1 ] << ", "
                 << cutVolumeVVFBB.second[ 2 ] << " ) ]" << LogEnd;
            auto cutVolumeVVFLocalizedBB = cutBoundingBox( maskVolume->boundingBox, volumeCutXYVVFLocalized, volumeCutZVVFLocalized );
            LogD << "Cut Volume Bounding Box for localized evaluation: [ ( " << cutVolumeVVFLocalizedBB.first[ 0 ] << ", "
                 ...
                 << cutVolumeVVFLocalizedBB.second[ 0 ] << ", " << cutVolumeVVFLocalizedBB.second[ 1 ] << ", " << cutVolumeVVFLocalizedBB.second[ 2 ]
                 << " ) ]" << LogEnd;
            ...
            // The fraction:
            globalVolumeData.vesselVolumeFraction = static_cast< Real >( vesselVoxelCount ) / static_cast< Real >( volumeVVFCylinderVoxelCount );

            // LOCAL !

            // We know the percentage of capillaries -> so we derive the fractions for capillaries and non-capillaries accordingly:
            // Keep in mind: this is not perfectly accurate ... but suffices for the statistics.
            ...
            globalVolumeData.vesselVolumeFractionLocalNonCapillary = globalVolumeData.vesselVolumeFractionLocal * nonCapFrac;
            ...
            LogD << "Volume contains " << vesselVoxelCount << " vessel voxels in a total of " << volumeVVFCylinderVoxelCount << " voxels."
                 << LogEnd;
            LogD << "Network-local Volume contains " << vesselVoxelLocalCount << " vessel voxels in a total of " << volumeVVFLocalCylinderVoxelCount
                 << " voxels." << LogEnd;
        }
        ...
        else
        {
            ...
            LogD << "Cut Volume Bounding Box for EVD: [ ( " << cutVolumeEVDBB.first[ 0 ] << ", " << cutVolumeEVDBB.first[ 1 ] << ", "
                 << cutVolumeEVDBB.first[ 2 ] << " ), ( " << cutVolumeEVDBB.second[ 0 ] << ", " << cutVolumeEVDBB.second[ 1 ] << ", "
                 << cutVolumeEVDBB.second[ 2 ] << " ) ]" << LogEnd;
            ...
            std::generate( globalEVDData.histBinsCenters.begin(), globalEVDData.histBinsCenters.end(), [&]()
                {
                    Real center = evdHistMin + ( evdHistBinWidth / 2.0 ) + ( static_cast< Real >( binCenterNum ) * evdHistBinWidth );
                    binCenterNum++;
                    return center;
                }
            );
            ...
            size_t evdReallyUsed = evdVoxelsUsed - evdIgnoredByHistogram - evdIgnoredByMinimum - evdIgnoredByMaximum - evdIgnoredByVolumeCrop;
            ...
            for( auto binNum : range( evdHistogram.size() ) )
            {
                ...
                LogD << "[ " << binLeft << ", " << binRight << " )" << " - " << perc << " ( " << evdHistogram[ binNum ] << " )" << LogEnd;
                evdHistogramPerc[ binNum ] = perc;
            }

            globalEVDData.mean /= static_cast< Real >( evdVoxelsUsed );
            LogI << "Calculated EVD histogram. Mean is " << globalEVDData.mean << ", max is " << globalEVDData.max << "." << LogEnd;
